...
class Vector {
    ...
    static randomDirection() {
        return Vector.fromAngles(Math.random() * constants_1.TWO_PI, Math.asin(Math.random() * 2 - 1));
    }
    static min(a, b) {
    ...
    static toDegrees(a) {
        return a * (180 / constants_1.PI);
    }
    static normalizeAngle(radians) {
        let angle = radians % constants_1.TWO_PI;
        angle = angle > constants_1.PI ? angle - constants_1.TWO_PI : angle < -constants_1.PI ? constants_1.TWO_PI + angle : angle;
        //returns normalized values to -1,1
        return angle / constants_1.PI;
    }
    static normalizeRadians(radians) {
        if (radians >= constants_1.PI / 2) {
            radians -= constants_1.TWO_PI;
        }
        if (radians <= -constants_1.PI / 2) {
            radians += constants_1.TWO_PI;
            radians = constants_1.PI - radians;
        }
        //returns normalized values to -1,1
        return radians / constants_1.PI;
    }
    ...
    static getSphericalCoords(a, b, axisMap = { x: "x", y: "y", z: "z" }) {
        ...
        return {
            theta: Vector.normalizeAngle(-theta),
            phi: Vector.normalizeAngle(constants_1.PI / 2 - phi),
        };
    }
}
